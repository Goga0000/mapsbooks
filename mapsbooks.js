...
// Основной обработчик данных (processData) и функция updateMap остаются почти без изменений,
// за исключением того, что в marker list и на карте используются только базовые поля (Name, Country, City, Address, координаты).
window.processData = function (data) {
  ...
  ymaps.ready(function () {
    ...
    // Определяем переменные для фильтров и DOM-элементов
    var selectedCountry = null,
        ...
        countryTriggerTextEl = document.querySelector(".country-list-trigger-text"),
        cityTriggerTextEl = document.querySelector(".city-list-trigger-text"),
    ...
    // Функции обновления списков фильтров (страны, города) – без дополнительных данных
    ...
    // Функция обновления карты и списка точек – здесь используются только базовые данные
    function updateMap() {
      ...
      // Для каждого элемента загружаем координаты и создаём метку
      allPoints.forEach(function (item) {
        function processItem(coords) {
          ...
          // Добавляем в список маркеров только базовую информацию
      ...
      // Центрирование карты по найденным точкам
      setTimeout(function () {
    ...
    // -------------------
    // В этой версии критичные данные (имя, флаг, адрес) устанавливаются сразу,
    // а дополнительные (email, phone, work time, ссылки) подгружаются асинхронно.
    function openModal(item) {
      var modal = document.querySelector('.maps-item-content');
      modal.style.display = 'flex';
      // Сразу устанавливаем основные данные:
      ...
      // Скрываем блоки дополнительных данных до их загрузки:
      var workTimeWrapper = modal.querySelector('.maps-item-content-time-wrapper'),
          phoneWrapper = modal.querySelector('.maps-item-phone-wrapper'),
          mailWrapper = modal.querySelector('.maps-item-mail-wrapper');
      workTimeWrapper.style.display = "none";
      phoneWrapper.style.display = "none";
      mailWrapper.style.display = "none";
  
      // Асинхронно подгружаем дополнительные данные в фоне:
      setTimeout(function () {
        // Обработка ссылок
        ...
        if (item["Work time"] && item["Work time"].trim() !== "") {
          ...
          var workTimeContainer = modal.querySelector('.maps-item-content-time-contain');
    ...
    // -------------------
    // Привязка событий для кнопки Back – закрывает модальное окно и сбрасывает центр карты
    // -------------------
